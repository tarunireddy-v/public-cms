...
export default function Login() {
    ...
    useEffect(() => {
        ...
        const fetchActiveCases = async () => {
            try {
                const response = await fetch('http://localhost:5001/api/dashboard/active-cases', {
                    headers: { 'Content-Type': 'application/json' },
                });
                const data = await response.json().catch(() => ({}));
                if (!response.ok) {
                    throw new Error(data.message || 'Failed to fetch active cases');
                }

                if (isMounted) {
                    setActiveCases(typeof data.activeCases === 'number' ? data.activeCases : 0);
                }
            } catch (err) {
                console.error('Active cases fetch failed:', err.message);
            }
        };
                ...
                <div style={{ backgroundColor: 'rgba(255, 255, 255, 0.1)', border: '1px solid rgba(255, 255, 255, 0.2)', borderRadius: 'var(--radius-lg)', padding: '2rem', backdropFilter: 'blur(10px)', opacity: 0.8, maxWidth: '500px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
                        <div style={{ flex: 1, background: 'rgba(0,0,0,0.2)', borderRadius: 'var(--radius)', padding: '1rem' }}>
                            ...
                            <div style={{ fontSize: '1.5rem', fontWeight: 'bold', marginTop: '0.5rem' }}>{activeCases.toLocaleString()}</div>
                        </div>
                        <div style={{ flex: 1, background: 'rgba(0,0,0,0.2)', borderRadius: 'var(--radius)', padding: '1rem' }}>
